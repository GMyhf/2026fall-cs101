...
| 9 | **机考规格** | 三份样卷（W05 / W14 / W16）均为 6 题 / 112 分钟；全仓库无「约 120 分钟」「2 小时」等过时写法；题目标题、难度梯度和讲义正文均不得出现固定分值分配 |
| 10 | **版面标记** | 放映稿的非等宽文字里不得出现 `**` / 反引号（源里的记号不该印出来）；bullet 不得以空白（含全角空格）开头 |
| 11 | **列对齐** | 等宽（`ascii`）图里不得有跨过不同中文字数的对齐列 —— 汉字宽 1 em、Consolas 约 0.55 em，靠补空格对不出来。7 张历史遗留在 `KNOWN_RAGGED` 里挂账（见 T-018），新写的立刻变红 |
