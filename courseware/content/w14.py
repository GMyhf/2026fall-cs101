...
META = {
    ...
    'subtitle': 'LLM 原理 · 幻觉与验证 · 提示词 · 月考六题讲评与错误归因 · 复习清单',
...
print(min_capacity([1, 2, 3, 4, 5], 3))      # 6
''', '判据是 <= k 不是 == k：层数不足时再切一刀，承重只会更小'),

    ('table', 'T5 错误归因', [
        ['错法', '后果'],
        ['二分下界写 0 或 1', 'cur + x > cap 恒成立，永远不可行 -> 答案错 / 死循环'],
        ['mid 用 (lo+hi+1)//2 却配 hi = mid', '取整方向与收缩方向不匹配 -> 死循环 TLE'],
        ['判据写成 shelves(mid) == k', '恰好 k 层的最优解被误判不可行 -> 偏大 WA'],
    ...
    for i, (op, a, b) in enumerate(rels, 1):
        # 说好是朋友却已推出是敌人，或反过来 —— 就是矛盾
        clash = find(a) == (find(b + n) if op == 'F' else find(b))
        if clash:
            bad = bad or i
            continue                     # 矛盾的关系不采纳
        if op == 'F':
    ...
    return bad, len({find(i) for i in range(1, n + 1)})


print(camps(5, [('F', 1, 2), ('E', 2, 3), ('E', 3, 4), ('F', 4, 5)]))   # (0, 2)
''', '朋友同向合并，敌人交叉合并；数团体只数 1..n 这半边的根'),

    ('table', 'T6 错误归因', [
        ['错法', '后果'],
        ['find 里不写路径压缩', '链式数据退化成 O(n)，10^5 条关系 -> TLE'],
        ['只开 n 个点，另用"敌人表"', '推不出"敌人的敌人是朋友" -> WA'],
        ['E a b 漏写 union(a+n, b)', '对称性丢失，部分矛盾查不出 -> WA'],
        ['判出矛盾后照样合并', '错误信息污染后续判断 -> 团体数 WA'],
        ['数团体时把 1..2n 全数一遍', '对立域被当成真人 -> 个数翻倍 WA'],
    ], '关系里出现"否定"且否定之间还能推理，就开对立域 —— 与 T3"状态加一维"同一类判断'),
    ...
    ('bullets', '小结', [
        ...
        '月考六题 → 六个高频坑（T1-T3）：**并列不处理、排序键靠猜、状态少一维**',
        '（T4-T6）：**"恰好"没用 inf、二分下界写错、并查集不写路径压缩**',
