...
META = {
    ...
    'subtitle': '知识清单自检 · 月考样卷 5 题 · 考后订正方法 · 考场策略',
...
SLIDES = [
    ...
    ('table', '考试形式', [
        ['项目', '说明'],
        ['地点 / 时长', '机房（安排以通知为准）；约 2 小时'],
        ['题量 / 平台', '5 题；OpenJudge（cs101 小组）'],
        ['语言', 'Python 3 为主，允许 C++'],
...
''', ''),
    ...
    ('section', '第 2 节', '月考样卷（5 题 / 100 分）'),

    ('ascii', '难度梯度', r"""
   T1  15分  *          签到：读入与格式化输出        -- 95% 应 AC
   T2  20分  **         字符串处理                    -- 75% AC
   T3  20分  ***        字典 / 排序 + 多关键字         -- 55% AC
   T4  20分  ***        复杂度意识：必须用筛或前缀和   -- 45% AC
   T5  25分  ****       综合模拟，边界多              -- 25% AC
"""),
...
''', '格式必须是 80.00，不能是 80.0'),
    ...
    ('code', 'T2 单词首字母大写（20 分）', '''import sys
...
''', "⚠️ 不能直接用 s.title()：它会把 don't 变成 Don'T。内建函数的边界行为要自己验证"),
    ...
    ('code', 'T3 图书借阅排行（20 分）', '''import sys
...
''', '卡 O(n^2)：n=2x10^5 只用 500 个不同书名，用 list.count() 逐个统计必 TLE'),
    ...
    ('code', 'T4 区间内的 T-数（20 分）', '''import sys
...
''', '恰好 3 个约数 <=> 素数的平方（约数个数 = (a1+1)(a2+1)... = 3 只能是 p^2）'),
    ...
    ('code', 'T5 电梯调度模拟（25 分）', '''import sys
