...
''', '不能按字典序，也不能按数值：正确比较是 a+b 与 b+a 谁大'),
    ...
    ('bullets', '交换论证的三步骨架', [
        '**1. 取任意一个最优解 OPT**，不假设它长什么样',
        '**2. 若 OPT 里相邻两项 a、b 与规则 R 相反，就交换**，并证明目标函数不会变差',
        '**3. 每换一次逆序对至少减 1**，逆序对有限 → 有限步后变成 R 序，且一路没变差',
        '结论是「**最优值能被 R 序取到**」，不是「R 序是唯一最优解」—— 贪心只需要前者',
        '**为什么必须相邻**：只有相邻交换才保证其余各项的贡献不变，'
        '差值里才只剩 a、b 互相的那一项',
        '隔项交换会把中间那项的变化也卷进来，**化简不出干净的不等式**',
    ]),

    ('key', '反过来用：这才是它在考场上的价值',
     '设相邻两项 a、b，写出「a 在前」与「b 在前」的代价，令前者不劣 —— '
     '化简出来的不等式就是排序键。'),

    ('code', '12559 的排序键是这么推出来的', '''# a、b 相邻交换只改变拼接串里那一段，
# 而 a+b 与 b+a 等长 => 整串比较退化成这两段比较
#   a 在前 -> a+b        b 在前 -> b+a
# 所以：a 应排在 b 前面  <=>  a + b > b + a
import functools
import itertools
import random
...
print(''.join(sorted(nums, key=int, reverse=True)))     # 3430953  数值序，错

random.seed(6)
for _ in range(300):                                    # 推完必须对拍
    t = [str(random.randint(0, 99)) for _ in range(random.randint(2, 6))]
    assert by_rule(t) == max(''.join(p) for p in itertools.permutations(t))
''', '推导会出错，对拍不会 —— 300 组随机数据与全排列暴力逐组一致'),

    ('table', '两种形态，别混', [
        ['形态', '用在哪', '交换什么'],
        ['相邻交换', '排序型贪心（12559）', 'OPT 里相邻的两项'],
        ['首元素替换', '选择型贪心（第 10 周区间问题）', 'OPT 的第一个选择换成贪心的选择'],
    ], '第 10 周会用后一种证「按右端点排」：把 OPT 的第一个区间换成右端点最小的那个，之后能放的只多不少'),

    ('bullets', '两个必须自己检查的前提', [
        '**一、规则 R 必须是全序**（传递性）—— 否则「R 序」根本没有定义，'
        '「逆序对递减」也就垮了',
        '`a+b > b+a` 的传递性可以证明；**换一道题要重新验**',
        '**浮点数当排序键最容易在这里翻车**：本该相等的两个键因误差变得一大一小',
        '**二、「交换只影响这两项」要真的成立** —— 这是相邻交换全部的立足点',
        '题目里若有跨项耦合（代价依赖全局某个量），这一步必须重算，不能照抄',
        '交换论证不是事后安慰自己「大概是对的」，是**动手之前把排序键算出来**',
    ]),
...
''', ''),
    ...
    '**排序键要用交换论证推，不要猜**：写出相邻两项两种摆法的代价，令「a 在前」不劣，化简出的不等式就是 `key`；推完必须对拍',
