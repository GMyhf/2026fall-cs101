...
| 前提 | 含义 | 反例 |
| ---- | ---- | ---- |
| **最优子结构** | 大问题的最优解由子问题的最优解构成 | 最长路（有环时不成立） |
| **重叠子问题** | 同一个子问题被反复求解 | 归并排序（子问题不重叠，用分治即可） |

**重叠子问题**是 DP 相对于分治的关键区别：分治的子问题互不相同，DP 的子问题被反复用到，
所以要**把答案存下来**。
...
3. DP 的两个前提：**最优子结构 + 重叠子问题**。没有重叠就用分治，没有最优子结构就不能 DP。
