...
> **回看第 10 周 §2.2**：那里说"没有最优子结构，通常只是状态少了一维"，
> 并欠了一个例子 —— 现在有图的语言了，可以补上。
>
> **一般图上的最长简单路径**（不能重复经过顶点）是真的没有最优子结构：
> 设 s→t 的最长简单路径经过 v，那么它的前半段 s→v **不一定**是 s→v 的最长简单路径 ——
> 走一条更长的 s→v 可能会把 v→t 那段要用的顶点提前用掉，反而走不到 t。
> 子问题的最优解在父问题里**不可用**，而且这不是补一维能解决的：
> 你要记住的是"已经用过哪些顶点"，那是 2^V 个状态，等于放弃了 DP。
>
> 对照一下**最短路**：BFS / Dijkstra 之所以成立，正是因为最短路的每一段也都是最短路。
> **同一张图、只把"最长"换成"最短"，最优子结构就有了** ——
> 它确实是问题与状态的性质，不是"图"这个结构的性质。
