...
**知识点**：第 1–4 周知识清单自检、月考样卷（5 题，含题面 / 样例 / 参考解答 / 评分要点）、常见失分点归类、机房考试流程与考场策略、考后订正方法。
...
2. 打字与编辑速度够不够——同样的思路，2 小时能写完 5 题还是 2 题；
...
**分值分配**：T1 15 + T2 20 + T3 20 + T4 20 + T5 25 = **100 分**
...
- 状态机写法正确：15 分；用 `''.join()` 而非循环拼接字符串：5 分。
...
- 用 `Counter` 或 `dict` 一遍统计：10 分；多关键字排序 `(-count, name)` 正确：10 分。
...
- 想到"素数的平方"这一等价条件：8 分；筛法预处理：7 分；浮点开方校正：5 分。
...
- 主循环四步（移动 / 等待 / 开门 / 送达）齐全：15 分；
- 送达时刻的定义处理正确（不含最后开关门）：5 分；
- 用快速输入通过 n = 10⁵：5 分。
...
3. 样卷的难度梯度是 **签到 → 字符串 → 字典排序 → 复杂度意识 → 综合模拟**，
   对应第 1–4 周的全部内容。
